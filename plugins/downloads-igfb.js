...
try {
await m.react('🕒')
const api = `https://api.vreden.web.id/api/igdownload?url=${encodeURIComponent(args[0])}`
...
if (!data.length) {
try {
const api = `https://api.delirius.store/download/instagram?url=${encodeURIComponent(args[0])}`
const res = await fetch(api)
const json = await res.json()
